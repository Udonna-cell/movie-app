...
router.get("/:name", (req, res, next) => {
  ...
  let data = {
    seasons: seasons.map(()=>{count++; return {name: `Sesson ${count}`, value: count, checked: (count == 1)? true: false}}),
    episodes: seasons
  }
  console.log(data.seasons)
  console.log(req.cookies)
  res.render("detail", { movie , year: new Date().getFullYear(), rates, casts, data });
});
